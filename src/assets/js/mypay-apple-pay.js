...
console.log(wc_mypay_apple_pay_params.site_url + '/index.php/checkout/order-received/');
...
jQuery(document).ready(function ($) {
    ...
    if (wc_mypay_apple_pay_params.server_https == 'on') {
        if (wc_mypay_apple_pay_params.test_mode == 'yes') {
            console.log('s0:檢查是否為https');
            console.log(wc_mypay_apple_pay_params.server_https);
        }


        if (window.ApplePaySession) {

            var merchantIdentifier = wc_mypay_apple_pay_params.display_name;
...
function beginPayment() {
    ...
    var request = {
        ...
        total: {
            label: wc_mypay_apple_pay_params.lable,
            amount: wc_mypay_apple_pay_params.total 	/* amount為交易金額，商家需自行帶入此次交易的金額 */
        }
    ...
    session.onvalidatemerchant = function (event) {
        ...
        jQuery.ajax({
            type: "POST",
            url: wc_mypay_apple_pay_params.ajaxurl,
            data: data,
            dataType: "json",
    ...
    session.onpaymentauthorized = function (event) {
        ...
        var send = {
            ...
            order_id: wc_mypay_apple_pay_params.order_id
        };

        if (wc_mypay_apple_pay_params.test_mode == 'yes') {
            console.log('s5:進行付款，送出付款資訊到高鉅');
            console.log(send);
        }
        ...
        jQuery.ajax({
            ...
            success: function (merchantSession) {
                ...
                if (merchantSession.RtnCode == 1) {
                    ...
                    window.location.href = wc_mypay_apple_pay_params.site_url + "/index.php/checkout/order-received/";
                } else {
                    if (wc_mypay_apple_pay_params.test_mode == 'yes') {
